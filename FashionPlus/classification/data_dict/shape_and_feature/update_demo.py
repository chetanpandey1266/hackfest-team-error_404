...
def manually_update_feature(opt_feat, input_feature, swapped_partID):
    ...
    if argopt.update_full:
        ...
        opt_feat = torch.from_numpy(input_feature.feature).float()
...
def compute_updated_feature(input_feature):
    ...
    mask = torch.from_numpy(get_mask(input_feature.feature)).byte()
    opt_feat = torch.from_numpy(input_feature.feature).float()
    opt_feat.requires_grad = True
    ...
    while (n_iter <= argopt.max_iter_hr):
        ...
        if n_iter % argopt.display_freq == (argopt.display_freq-1):
            print('Iteration: %d, loss: %f'%(n_iter+1, loss.data))
...
with open('../../../generation/results/Lab/demo/test_features.p', 'rb') as readfile:
    piece_texture_feat_dict = pickle.load(readfile)
with open('../../../generation/results/Lab/demo/test_shape_codes.p', 'rb') as readfile:
...
if argopt.load_pretrain_clf:
    # print(argopt.load_pretrain_clf)
    load_network(argopt, model, argopt.clf_epoch, save_dir='../../../checkpoint')
